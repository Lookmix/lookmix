...
export function setLocalStorageTokenData(dados){
	...
	if(environment.production) {
		dados['access_token_data']['expires'] = new Date(expiresAccessToken).toLocaleString('pt-BR');
		dados['refresh_token_data']['expires'] = new Date(expiresRefreshToken).toLocaleString('pt-BR');
	}	else {
		dados['access_token_data']['expires'] = new Date(expiresAccessToken.replace(' GMT', '')).toLocaleString('pt-BR');
		dados['refresh_token_data']['expires'] = new Date(expiresRefreshToken.replace(' GMT', '')).toLocaleString('pt-BR');
	}

	const access_token_data = JSON.stringify(dados["access_token_data"]);
	const refresh_token_data = JSON.stringify(dados["refresh_token_data"]);

	localStorage.setItem("access_token_data", access_token_data);
	localStorage.setItem("refresh_token_data",refresh_token_data);
}
