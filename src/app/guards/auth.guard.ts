...
export class AuthGuard implements CanActivate {
  private whiteListRoutes = ['login','nova-conta'];
  ...
  canActivate(
    ...
    if (this.whiteListRoutes.includes(url)) return this.permitirNavegarParaLoginOuNovaConta();

    if (utils.isTokenValid('access_token_data')){

      const localStorageDadosUrlAnterior = utils.getFromLocalStorageDadosUrlAnterior();
      if (localStorageDadosUrlAnterior) utils.setInLocalStorageDadosUrlAnterior(localStorageDadosUrlAnterior['url'], false);
      return true;
    ...
    this.segurancaService.refreshToken()
      .subscribe(
        ...
        error => {
          ...
          this.router.navigate(['login']);
          dialogSpinnerRef.close();
          this.snackBar.open('Devido à um erro de rede você precisa entrar novamente.','', {duration: 4500});
          console.log(error);
        });
  }
  ...
  private permitirNavegarParaLoginOuNovaConta(): boolean {
    if(utils.isTokenValid('access_token_data') || utils.isTokenValid('refresh_token_data')){
